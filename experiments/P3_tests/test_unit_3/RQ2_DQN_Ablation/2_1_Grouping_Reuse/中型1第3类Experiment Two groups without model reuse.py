...
# 动作步长（所有维度均为整数，步长可统一使用 1，但保留原有多样性）
STEP_VALUES = [1, 2, 3, 5, -1, -2, -3, -5]  # 与原交通域一致

# === 归一化函数 ===
def normalize_state(state):
    """归一化到 [0,1]"""
    x, y, z = state
    norm_x = (x - PATH_DEPTH_MIN) / (PATH_DEPTH_MAX - PATH_DEPTH_MIN)
    norm_y = (y - FILE_COUNT_MIN) / (FILE_COUNT_MAX - FILE_COUNT_MIN)
    norm_z = (z - ACCESS_LEVEL_MIN) / (ACCESS_LEVEL_MAX - ACCESS_LEVEL_MIN)
    return [norm_x, norm_y, norm_z]

def denormalize_state(normalized_state):
    """反归一化，所有维度取整"""
    nx, ny, nz = normalized_state
    x = int(np.clip(nx * (PATH_DEPTH_MAX - PATH_DEPTH_MIN) + PATH_DEPTH_MIN,
                    PATH_DEPTH_MIN, PATH_DEPTH_MAX))
    y = int(np.clip(ny * (FILE_COUNT_MAX - FILE_COUNT_MIN) + FILE_COUNT_MIN,
                    FILE_COUNT_MIN, FILE_COUNT_MAX))
    z = int(np.clip(nz * (ACCESS_LEVEL_MAX - ACCESS_LEVEL_MIN) + ACCESS_LEVEL_MIN,
                    ACCESS_LEVEL_MIN, ACCESS_LEVEL_MAX))
    return [x, y, z]
...
def compute_robustness(state, path):
    ...
    if not base:
        return 0.0
    rob, neighbors = 0.0, 0
    # 使用与动作相同的步长集合（但这里只做小范围邻域，取 -1,0,1 即可，但为统一保留原样）
    for dx in [-1, 0, 1]:
        for dy in [-1, 0, 1]:
            for dz in [-1, 0, 1]:
                if dx == dy == dz == 0:
                    continue
                neighbor = [
                    np.clip(state[0] + dx, PATH_DEPTH_MIN, PATH_DEPTH_MAX),
                    np.clip(state[1] + dy, FILE_COUNT_MIN, FILE_COUNT_MAX),
                    np.clip(state[2] + dz, ACCESS_LEVEL_MIN, ACCESS_LEVEL_MAX)
                ]
                ...
                for sample_idx in range(batch_start, batch_end):
                    ...
                    for step in range(N_STEPS):
                        ...
                        for a in range(agent.action_dim):
                            ...
                            if (PATH_DEPTH_MIN <= cand_next[0] <= PATH_DEPTH_MAX and
                                FILE_COUNT_MIN <= cand_next[1] <= FILE_COUNT_MAX and
                                ACCESS_LEVEL_MIN <= cand_next[2] <= ACCESS_LEVEL_MAX):
                        ...
                        next_state = (state[0] + dp, state[1] + df, state[2] + da)
...
def create_consolidated_excel_report(all_runs_data, similar_group, isolated_group, output_dir):
    ...
    output_path = os.path.join(output_dir, "20次运行_编排规则.xlsx")
    wb.save(output_path)
...
def run_20_times_training():
    ...
    print(f"范围: path_depth[{PATH_DEPTH_MIN},{PATH_DEPTH_MAX}], file_count[{FILE_COUNT_MIN},{FILE_COUNT_MAX}], access_level[{ACCESS_LEVEL_MIN},{ACCESS_LEVEL_MAX}]")
    ...
    print("\n" + "=" * 60)
    print("20次运行全部完成！ - 编排规则")
    print("=" * 60)
    print(f"状态空间范围:")
    print(f"  path_depth: [{PATH_DEPTH_MIN}, {PATH_DEPTH_MAX}]")
    print(f"  file_count: [{FILE_COUNT_MIN}, {FILE_COUNT_MAX}]")
    print(f"  access_level: [{ACCESS_LEVEL_MIN}, {ACCESS_LEVEL_MAX}]")
