...
STATE_NAMES = ('path_depth', 'file_count', 'access_level')
...
def clip_state(state):
    """裁剪状态到合法范围，全部取整（所有维度均为整数）"""
    state = np.array(state, dtype=np.int32)
    clipped = np.clip(state, STATE_MIN, STATE_MAX)
    return tuple(clipped.astype(int))

def random_state():
    """生成随机状态：三个整数"""
    pd = random.randint(int(STATE_RANGES['path_depth'][0]), int(STATE_RANGES['path_depth'][1]))
    fc = random.randint(int(STATE_RANGES['file_count'][0]), int(STATE_RANGES['file_count'][1]))
    al = random.randint(int(STATE_RANGES['access_level'][0]), int(STATE_RANGES['access_level'][1]))
    return (pd, fc, al)
...
class StateNormalizer:
    ...
    def denormalize(self, normalized_state):
        ...
        denormalized[2] = int(round(np.clip(denormalized[2], low, high)))
        return denormalized
...
def execute_orchestration_rules(a):
    """
    参数a: 包含3个元素的元组或数组，对应 path_depth, file_count, access_level
    返回: 触发的规则编号集合
    """
...
def compute_robustness(state, path):
    """计算鲁棒性：邻域平均Jaccard相似度（适配编排规则，所有维度步长为1）"""
    pd, fc, al = state
    base = execute_Tr(pd, fc, al)
    if not base:
        return 0.0
    ...
    def save_samples(path_id, samples, base_dir, group_type="similar"):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            for s in samples:
                pd, fc, al = s['state']
                f.write(
                    f"{pd} {fc} {al}\t{s['score']:.4f}\t{s['similarity']:.4f}\t{s['length_diff']:.4f}\t{s['robustness']:.4f}\n")
    ...
    def save_samples(path_id, samples, base_dir, group_type="isolated"):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            for s in samples:
                pd, fc, al = s['state']
                f.write(
                    f"{pd} {fc} {al}\t{s['score']:.4f}\t{s['similarity']:.4f}\t{s['length_diff']:.4f}\t{s['robustness']:.4f}\t{s['q_value_score']:.4f}\n")
...
def create_consolidated_excel_report(all_runs_data, similar_group, isolated_group, output_dir):
    ...
    sample_headers = ['Run', 'Path ID', 'Sample ID', 'Path_Depth', 'File_Count', 'Access_Level', 'Similarity', 'Triggered Rule Set']
    ...
    output_path = os.path.join(output_dir, "20_run_orchestration_random_reuse.xlsx")
    wb.save(output_path)
    print(f"\n Consolidated Excel report generated: {output_path}")

def run_20_times_training():
    model_path_base = r"D:\Experiment\CNN\DQNNEW\saved_models_orchestration_random_reuse"
    path_documents = r"D:\Experiment\CNN\DQNNEW\path_samples_grouped"
    output_dir = r"D:\Experiment\CNN\ComparisonExperiment2\excel_reports_orchestration_random_reuse"
    ...
    print("=" * 60)
    print("20 - Orchestration rules + random grouping + model reuse")
    print(f"State ranges: path_depth[1,15], file_count[1,110000], access_level[1,4]")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("20 runs all completed! - Orchestration rules + random grouping + model reuse")
    print("=" * 60)
    print(f"Summary:")
