...
EXPERIMENT_CONFIG = {
    'STATE_DIM': 3,
    'ACTION_DIM': 3,
    # 修改后的状态范围：path_depth[1,15], file_count[1,110000], access_level[1.0,4.0]
    'MIN_VALUES': np.array([1, 1, 1.0], dtype=np.float32),
    'MAX_VALUES': np.array([15, 110000, 4.0], dtype=np.float32),
    ...
    def get_top_k_per_path(self, num_paths, k=20):
        ...
        for path_idx in range(num_paths):
            ...
            for sample in top_k:
                ...
                original_state[2] = original_state[2]
...
def main():
    ...
    min_vals = EXPERIMENT_CONFIG['MIN_VALUES']
    max_vals = EXPERIMENT_CONFIG['MAX_VALUES']
    print(f"状态范围: path_depth[{min_vals[0]}-{max_vals[0]}], "
          f"file_count[{min_vals[1]}-{max_vals[1]}], "
          f"access_level[{min_vals[2]}-{max_vals[2]}]")
    print("=" * 80)
