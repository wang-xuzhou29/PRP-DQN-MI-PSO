...
# ==================== 编排规则专用范围配置（新范围） ====================
PATH_DEPTH_MIN, PATH_DEPTH_MAX = 1, 30
FILE_COUNT_MIN, FILE_COUNT_MAX = 1, 40
ACCESS_LEVEL_MIN, ACCESS_LEVEL_MAX = 1, 2100

# === 归一化/反归一化函数 ===
def normalize_state(state):
    """将状态归一化到 [0,1]"""
    pd, fc, al = state
    pd_norm = (pd - PATH_DEPTH_MIN) / (PATH_DEPTH_MAX - PATH_DEPTH_MIN)
    fc_norm = (fc - FILE_COUNT_MIN) / (FILE_COUNT_MAX - FILE_COUNT_MIN)
    al_norm = (al - ACCESS_LEVEL_MIN) / (ACCESS_LEVEL_MAX - ACCESS_LEVEL_MIN)
    return (pd_norm, fc_norm, al_norm)

def denormalize_state(state_norm):
    """将归一化状态还原，所有维度取整"""
    pd_n, fc_n, al_n = state_norm
    pd = int(round(pd_n * (PATH_DEPTH_MAX - PATH_DEPTH_MIN) + PATH_DEPTH_MIN))
    fc = int(round(fc_n * (FILE_COUNT_MAX - FILE_COUNT_MIN) + FILE_COUNT_MIN))
    al = int(round(al_n * (ACCESS_LEVEL_MAX - ACCESS_LEVEL_MIN) + ACCESS_LEVEL_MIN))
    pd = np.clip(pd, PATH_DEPTH_MIN, PATH_DEPTH_MAX)
    fc = np.clip(fc, FILE_COUNT_MIN, FILE_COUNT_MAX)
    al = np.clip(al, ACCESS_LEVEL_MIN, ACCESS_LEVEL_MAX)
    return (pd, fc, al)
...
def compute_robustness(state, path, sample_size=9):
    ...
    rob, neighbors = 0.0, 0
    # 针对新范围的邻域步长（path_depth ±1, file_count ±1, access_level ±100）
    deltas = [
    ...
    for dp, df, da in deltas[:sample_size]:
        if dp == df == da == 0:
            continue
        neighbor_pd = int(np.clip(state[0] + dp, PATH_DEPTH_MIN, PATH_DEPTH_MAX))
        neighbor_fc = int(np.clip(state[1] + df, FILE_COUNT_MIN, FILE_COUNT_MAX))
        neighbor_al = int(np.clip(state[2] + da, ACCESS_LEVEL_MIN, ACCESS_LEVEL_MAX))
        neighbor = (neighbor_pd, neighbor_fc, neighbor_al)
...
def generate_samples_for_all_paths(num_candidates=2000, top_k=200, run_id=1):
    ...
    def save_samples(path_id, samples, base_dir):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write("path_depth file_count access_level\tScore\tSimilarity\tRobustness\tLengthDiff\n")
            for s in samples:
                pd, fc, al = s['state']
                f.write(
                    f"{pd} {fc} {al}\t{s['score']:.4f}\t{s['similarity']:.4f}\t"
...
class DQNAgentWithPER:
    ...
    def decode_action(self, action_idx):
        """
        将动作索引解码为三维步长（每个维度10种动作，共30维）
        针对新范围设计的步长：
        - path_depth:   [-6,-5,-4,-3,-2,0,2,3,4,5]
        - file_count:   [-6,-5,-4,-3,-2,0,2,3,4,5]
        - access_level: [-600,-500,-400,-300,-200,0,200,300,400,500]
        """
        step_lists = [
            [-6, -5, -4, -3, -2, 0, 2, 3, 4, 5],                         # path_depth
            [-6, -5, -4, -3, -2, 0, 2, 3, 4, 5],                         # file_count
            [-600, -500, -400, -300, -200, 0, 200, 300, 400, 500]        # access_level
        ]
    ...
    def get_legal_actions(self, state):
        ...
        for action_idx in range(self.action_dim):
            dp, df, da = self.decode_action(action_idx)
            next_pd = state[0] + dp
            next_fc = state[1] + df
            next_al = state[2] + da
            if (PATH_DEPTH_MIN <= next_pd <= PATH_DEPTH_MAX and
                FILE_COUNT_MIN <= next_fc <= FILE_COUNT_MAX and
                ACCESS_LEVEL_MIN <= next_al <= ACCESS_LEVEL_MAX):
            ...
            for batch_idx in range(NUM_BATCHES):
                ...
                for sample_idx in range(batch_start, batch_end):
                    ...
                    for step in range(STEPS_PER_SAMPLE):
                        ...
                        next_state = (
                            int(np.clip(state[0] + dp, PATH_DEPTH_MIN, PATH_DEPTH_MAX)),
                            int(np.clip(state[1] + df, FILE_COUNT_MIN, FILE_COUNT_MAX)),
                            int(np.clip(state[2] + da, ACCESS_LEVEL_MIN, ACCESS_LEVEL_MAX))
                        )
...
def create_consolidated_excel_report(all_runs_data, similar_group, isolated_group, output_dir):
    ...
    sample_headers = ['Run', 'Path ID', 'Sample ID', 'Path_Depth', 'File_Count',
                      'Access_Level', 'Similarity', 'Triggered Rules']
    ...
    output_path = os.path.join(output_dir, "20_runs_orchestration_new_range.xlsx")
    wb.save(output_path)
...
def run_20_times_training():
    model_path_base = r"D:\Experiment\CNN\DQNNEW\saved_models_orchestration_new"
    path_documents = r"D:\Experiment\CNN\DQNNEW\path_samples_individual"
    output_dir = r"D:\Experiment\CNN\ComparisonExperiment2\excel_reports_orchestration_new"
    ...
    print("=" * 60)
    print("20次运行 - 编排规则（新范围）")
    print(f"状态范围: path_depth[{PATH_DEPTH_MIN},{PATH_DEPTH_MAX}], "
    ...
    print("动作空间: 30维 (每维度10种步长，已适配新范围)")
    print("=" * 60)
