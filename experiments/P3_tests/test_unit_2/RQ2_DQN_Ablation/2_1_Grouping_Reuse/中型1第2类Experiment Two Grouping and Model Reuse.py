...
class StateNormalizer:
    ...
    def normalize(self, state):
        ...
        normalized[0] = (state[0] - self.ranges['config_depth'][0]) / (self.ranges['config_depth'][1] - self.ranges['config_depth'][0])
        normalized[1] = (state[1] - self.ranges['param_count'][0]) / (self.ranges['param_count'][1] - self.ranges['param_count'][0])
        normalized[2] = (state[2] - self.ranges['security_weight'][0]) / (self.ranges['security_weight'][1] - self.ranges['security_weight'][0])
        return normalized

    def denormalize(self, normalized_state):
        """反归一化并四舍五入为整数（针对 config_depth 和 param_count）"""
        normalized_state = np.array(normalized_state, dtype=np.float32)
        denormalized = np.zeros_like(normalized_state)
        denormalized[0] = normalized_state[0] * (self.ranges['config_depth'][1] - self.ranges['config_depth'][0]) + self.ranges['config_depth'][0]
        denormalized[1] = normalized_state[1] * (self.ranges['param_count'][1] - self.ranges['param_count'][0]) + self.ranges['param_count'][0]
        denormalized[2] = normalized_state[2] * (self.ranges['security_weight'][1] - self.ranges['security_weight'][0]) + self.ranges['security_weight'][0]
        # config_depth 和 param_count 取整
        denormalized[0] = np.clip(np.round(denormalized[0]), self.ranges['config_depth'][0], self.ranges['config_depth'][1]).astype(int)
        denormalized[1] = np.clip(np.round(denormalized[1]), self.ranges['param_count'][0], self.ranges['param_count'][1]).astype(int)
        denormalized[2] = np.clip(denormalized[2], self.ranges['security_weight'][0], self.ranges['security_weight'][1])
        return denormalized

normalizer = StateNormalizer()

# === 执行验证规则函数（与之前完全一致，只改变输入参数的映射） ===
def execute_validation_rules(a):
    """
    参数 a: 包含 config_depth, param_count, security_weight 的元组/数组
    返回: 触发的规则编号集合
    """
...
# 为兼容原代码，设置别名（原代码中可能使用 execute_Tr）
...
def generate_samples_for_similar_paths(similar_group, num_candidates=2000, top_k=200, run_id=1):
    ...
    def save_samples(path_id, samples, base_dir, group_type="similar"):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            for s in samples:
                config_depth, param_count, security_weight = s['state']
                f.write(f"{config_depth} {param_count} {security_weight:.2f}\t{s['score']:.4f}\t{s['similarity']:.4f}\t{s['length_diff']:.4f}\t{s['robustness']:.4f}\n")
    ...
    for path_idx in similar_group:
        ...
        while len(candidate_samples) < num_candidates and attempts < num_candidates * 10:
            attempts += 1
            config_depth = random.randint(CONFIG_DEPTH_MIN, CONFIG_DEPTH_MAX)
            param_count = random.randint(PARAM_COUNT_MIN, PARAM_COUNT_MAX)
            security_weight = round(random.uniform(SEC_WEIGHT_MIN, SEC_WEIGHT_MAX), 2)
...
def generate_samples_for_isolated_paths(isolated_group, similar_model, num_candidates=2000, top_k=200, run_id=1):
    ...
    def save_samples(path_id, samples, base_dir, group_type="isolated"):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            for s in samples:
                config_depth, param_count, security_weight = s['state']
                f.write(f"{config_depth} {param_count} {security_weight:.2f}\t{s['score']:.4f}\t{s['similarity']:.4f}\t{s['length_diff']:.4f}\t{s['robustness']:.4f}\t{s['q_value_score']:.4f}\n")
    base_dir = r"D:\Experiment\CNN\DQNNEW\path_samples_grouped"
...
class GroupExperienceReplay:
    ...
    def get_high_reward_samples(self, target_path, num_samples=20):
        ...
        for idx, experience in enumerate(self.buffer):
            ...
            config_depth, param_count, security_weight = state_tuple
            triggered = execute_Tr(config_depth, param_count, security_weight)
...
def load_path_data(file_path):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        for line in lines[2:]:
            ...
            state = tuple(map(float, parts[0].split()))  # 读取浮点数
            # 转换为整数（config_depth, param_count）和浮点数（security_weight）
                ...
                for sample_idx in range(batch_start, batch_end):
                    ...
                    for step in range(N_STEPS):
                        ...
                        for a in range(agent.action_dim):
                            dw, dt, ds = agent.decode_action(a)
                            cand_next = tuple(np.clip(np.array(state) + np.array([dw, dt, ds]),
                                                      [CONFIG_DEPTH_MIN, PARAM_COUNT_MIN, SEC_WEIGHT_MIN],
                                                      [CONFIG_DEPTH_MAX, PARAM_COUNT_MAX, SEC_WEIGHT_MAX]))
                            # 若安全权重超出范围则丢弃
                            if cand_next[2] < SEC_WEIGHT_MIN or cand_next[2] > SEC_WEIGHT_MAX:
                                continue
                        ...
                        dw, dt, ds = agent.decode_action(action)
                        next_state = tuple(np.clip(np.array(state) + np.array([dw, dt, ds]),
                                                   [CONFIG_DEPTH_MIN, PARAM_COUNT_MIN, SEC_WEIGHT_MIN],
                                                   [CONFIG_DEPTH_MAX, PARAM_COUNT_MAX, SEC_WEIGHT_MAX]))
                        next_state = (int(next_state[0]), int(next_state[1]), round(next_state[2], 2))
                        normalized_next_state = normalizer.normalize(next_state)
                        config_depth, param_count, security_weight = next_state
                        triggered = execute_Tr(config_depth, param_count, security_weight)
    ...
    for run_idx, run_data in enumerate(all_runs_data, 1):
        for path_id in range(1, len(target_paths)+1):
            ...
            for sample_idx, (state_tuple, reward, sim, triggered) in enumerate(samples, 1):
                ...
                cell.alignment = Alignment(horizontal="center", vertical="center")
                cell.border = thin_border
                for col_offset, value in enumerate([config_depth, param_count, security_weight]):
