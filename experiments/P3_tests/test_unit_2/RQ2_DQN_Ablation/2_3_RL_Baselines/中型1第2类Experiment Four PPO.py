...
EXPERIMENT_CONFIG = {
    'STATE_DIM': 3,
    'ACTION_DIM': 3,
    'MIN_VALUES': np.array([1, 1, 0.0], dtype=np.float32),   # config_depth, param_count, security_weight
    'MAX_VALUES': np.array([7, 180, 1.0], dtype=np.float32),
    ...
    def get_top_k_per_path(self, num_paths, k=20):
        ...
        for path_idx in range(num_paths):
            ...
            for sample in top_k:
                ...
                # 四舍五入取整（config_depth 和 param_count 为整数，security_weight 保留浮点）
                original_state_int = np.round(original_state).astype(int)
                # 但 security_weight 需要保留浮点，但这里我们将其作为整数传递？实际上规则函数接受浮点，但我们的状态是浮点，且 clipped 后可能需要调整。
                # 为了安全，我们保持浮点，但将前两个维度转为 int
                original_state[0] = int(round(original_state[0]))
                original_state[1] = int(round(original_state[1]))
                original_state[2] = original_state[2]  # 浮点
    ...
    for run_idx, (ppo_results, performance_data) in enumerate(zip(all_ppo_results, all_performance_data)):
        for path_idx in range(len(target_paths)):
            ...
            for sample_idx, sample in enumerate(samples):
                ...
                similarity = sample['similarity']
                triggered = sample['triggered']
                # 将状态转换为可读格式（config_depth, param_count, security_weight）
                all_ppo_detailed_data.append({
                    'Run': run_idx + 1,
                    'Path ID': path_idx + 1,
                    'Sample ID': sample_idx + 1,
                    'Config_Depth': int(state[0]),
                    'Param_Count': int(state[1]),
                    'Security_Weight': round(state[2], 2),
...
def main():
    print("\n" + "=" * 80)
    print("PPO - 20次运行实验 (验证规则)")
    print("性能指标：覆盖相似度")
    min_vals = EXPERIMENT_CONFIG['MIN_VALUES']
    max_vals = EXPERIMENT_CONFIG['MAX_VALUES']
    print(f"状态范围: config_depth[{min_vals[0]}-{max_vals[0]}], "
          f"param_count[{min_vals[1]}-{max_vals[1]}], "
    ...
    output_path = f"PPO_20_runs_validation_{timestamp}.xlsx"
