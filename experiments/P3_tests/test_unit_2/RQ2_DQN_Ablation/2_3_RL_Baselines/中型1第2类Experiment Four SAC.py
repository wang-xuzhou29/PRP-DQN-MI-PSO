...
EXPERIMENT_CONFIG = {
    'STATE_DIM': 3,
    'ACTION_DIM': 3,
    # 测试范围：config_depth[1,7], param_count[1,180], security_weight[0,1]
    'MIN_VALUES': np.array([1.0, 1, 0.0], dtype=np.float32),
    'MAX_VALUES': np.array([7.0, 180, 1.0], dtype=np.float32),
    ...
    'TARGET_PATHS': [
        # A1
        {3, 7, 8, 11, 12, 14, 16, 17, 19, 22, 23, 24, 28, 30, 32, 35, 37, 40, 43, 47, 49, 51, 55, 57, 61, 62, 63, 65, 66,
         68, 70, 72, 79, 81, 83, 84, 86, 96, 97, 98, 99, 100, 101, 102, 104},
        # A2
        {3, 7, 8, 11, 12, 14, 16, 17, 19, 24, 28, 30, 32, 35, 37, 40, 43, 47, 51, 52, 55, 58, 61, 62, 63, 65, 66, 68, 70,
         72, 75, 79, 82, 84, 86, 96, 97, 98, 99, 100, 101, 102, 104},
        # A3
        {3, 6, 8, 11, 13, 16, 19, 22, 23, 25, 27, 30, 32, 35, 37, 41, 43, 47, 48, 49, 51, 55, 57, 59, 60, 61, 62, 63, 68,
         70, 72, 73, 80, 81, 83, 86, 88, 89, 92, 96, 99, 100, 104},
        # A4
        {3, 7, 8, 11, 12, 19, 21, 24, 28, 30, 32, 33, 36, 40, 42, 45, 46, 47, 51, 52, 55, 58, 61, 62, 63, 65, 68, 70, 72,
         75, 79, 86, 93, 94, 96, 97, 98, 99, 100, 104},
        # A5
        ...
        {2, 3, 7, 8, 11, 12, 19, 20, 21, 24, 28, 30, 32, 33, 36, 40, 42, 45, 47, 50, 52, 55, 58, 63, 65, 68, 70, 72, 75, 79,
         86, 93, 94, 96, 104},
        # A7
        {1, 9, 10, 15, 18, 24, 26, 29, 31, 33, 34, 42, 44, 47, 49, 53, 54, 56, 64, 67, 69, 71, 74, 75, 77, 80, 82, 85, 88,
         90, 95, 98, 103},
        # A8
        {5, 7, 9, 10, 15, 18, 22, 24, 26, 29, 31, 33, 34, 38, 42, 44, 47, 60, 64, 67, 69, 71, 73, 81, 85, 89, 95, 103},
        # A9
        {4, 6, 8, 11, 13, 16, 19, 25, 27, 30, 32, 35, 37, 41, 43, 47, 51, 52, 55, 58, 60, 61, 62, 63, 68, 70, 72, 73, 76,
         78, 80, 82, 86, 88, 90, 96, 99, 100, 104},
        # A10
        {3, 7, 8, 11, 13, 16, 19, 22, 27, 28, 30, 32, 35, 37, 38, 39, 41, 43, 47, 49, 51, 56, 57, 63, 68, 70, 72, 81, 86,
         89, 96, 97, 98, 101, 104}
    ]
}
...
def execute_validation_rules(a):
    """
    配置验证规则执行函数
    参数a: 包含3个元素的数组，对应 config_depth, param_count, security_weight
    返回: 触发的规则编号集合
    """
    ...
    def get_top_k_per_path(self, num_paths, k=20):
        ...
        for path_idx in range(num_paths):
            ...
            for sample in top_k:
                ...
                # param_count取整，config_depth和security_weight保留浮点
                original_state[1] = int(round(original_state[1]))
        ...
        for path_idx in range(len(target_paths)):
            ...
            for sample_idx, sample in enumerate(samples):
                ...
                all_sac_detailed_data.append({
                    ...
                    'Config_Depth': round(state[0], 2),
                    'Param_Count': int(state[1]),
                    'Security_Weight': round(state[2], 4),
...
def train_sac_workflow():
    ...
    for path_idx in range(num_paths):
        ...
        for _ in range(EXPERIMENT_CONFIG['SAMPLES_PER_PATH']):
            # config_depth浮点，param_count整数，security_weight浮点
            state = np.array([
...
def main():
    print("\n" + "=" * 80)
    print("SAC - 20次运行实验 (配置验证规则)")
    print("性能指标：覆盖相似度")
    min_vals = EXPERIMENT_CONFIG['MIN_VALUES']
    max_vals = EXPERIMENT_CONFIG['MAX_VALUES']
    print(f"状态范围: config_depth[{min_vals[0]}-{max_vals[0]}], "
          f"param_count[{int(min_vals[1])}-{int(max_vals[1])}], "
          f"security_weight[{min_vals[2]}-{max_vals[2]}]")
    print("=" * 80)
    ...
    output_path = f"SAC_20_runs_validation_{timestamp}.xlsx"
