...
# === 修改后的三维范围（第二个单元测试）===
# 维度1: config_depth 1~7 (float)
LIGHT_MIN = 1.0
LIGHT_MAX = 7.0
# 维度2: param_count 1~180 (int)
MOISTURE_MIN = 1
MOISTURE_MAX = 180
# 维度3: security_weight 0~1 (float)
TEMP_MIN = 0.0
TEMP_MAX = 1.0

BOUNDS = {
    'light': (LIGHT_MIN, LIGHT_MAX),      # config_depth
    'temp': (TEMP_MIN, TEMP_MAX),         # security_weight
    'moisture': (MOISTURE_MIN, MOISTURE_MAX)  # param_count
}
...
def execute_Tr(position):
    """
    执行目标函数，返回触发的规则集合
    参数position: (config_depth, param_count, security_weight)
    """
    config_depth = float(np.clip(position[0], BOUNDS['light'][0], BOUNDS['light'][1]))
    param_count = int(np.clip(position[2], BOUNDS['moisture'][0], BOUNDS['moisture'][1]))
    security_weight = float(np.clip(position[1], BOUNDS['temp'][0], BOUNDS['temp'][1]))
    return execute_validation_rules(config_depth, param_count, security_weight)


def execute_validation_rules(config_depth, param_count, security_weight):
    """
    验证规则函数（来自第二个单元测试）
    参数：config_depth (float), param_count (int), security_weight (float)
    返回触发规则集合
    """
    triggered = set()
    b = {}

    # 以下代码从用户提供的第二个单元测试函数中完整复制
...
def generate_samples_for_similar_paths(similar_group_indices, num_total=2000, top_k=200):
    ...
    def save_samples(path_id, samples, base_dir):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            for s in samples:
                config_depth, param_count, security_weight = s[0][0], int(s[0][2]), s[0][1]
                f.write(f"{config_depth:.2f} {param_count} {security_weight:.3f}\t{s[1]:.4f}\t{s[2]:.4f}\t{s[3]:.4f}\t{s[4]:.4f}\n")
...
class SharedExperienceReplay:
    ...
    def get_high_reward_samples(self, target_path, num_samples=20):
        ...
        for experience in self.buffer:
            ...
            state_tuple = (state_np[0], state_np[2], state_np[1])  # (config_depth, param_count, security_weight)
...
def load_path_data(file_path):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        for line in lines[2:]:
            ...
            if parts:
                vals = parts[0].split()
                # 格式: config_depth param_count security_weight
                config_depth = float(vals[0])
                param_count = int(vals[1])
                security_weight = float(vals[2])
                path_data.append((config_depth, param_count, security_weight))
    return path_data
...
class DQNAgentWithPER:
    ...
    def decode_action(self, action_idx):
        """解码动作：0-9 为 config_depth，10-19 为 security_weight，20-29 为 param_count"""
        dim = action_idx // 10
        ...
        if dim == 0:  # config_depth
            ...
            return (delta, 0, 0)
        elif dim == 1:  # security_weight
            delta = DELTA_TEMP[delta_idx]
            return (0, delta, 0)
        elif dim == 2:  # param_count
            ...
            for epoch in range(N_EPOCHS):
                ...
                for batch_start in range(0, N_SAMPLES, BATCH_SIZE):
                    ...
                    for test_data_idx in range(batch_start, batch_end):
                        ...
                        # state_orig: (config_depth, param_count, security_weight)
                        state = (state_orig[0], state_orig[2], state_orig[1])  # 转换为 (light, temp, moisture) 顺序
                        prev_state = None
                        prev_triggered = None
                        prev_reward = None
...
def generate_samples_for_isolated_paths(agent_similar, isolated_group_indices, num_total=2000, top_k=200):
    ...
    def save_samples(path_id, samples, base_dir):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write("config_depth param_count security_weight\tScore\tSimilarity\tLengthDiff\tRobustness\tQ_value_normalized_complement\n")
            for s in samples:
                config_depth, param_count, security_weight = s[0][0], int(s[0][2]), s[0][1]
                f.write(f"{config_depth:.2f} {param_count} {security_weight:.3f}\t{s[1]:.4f}\t{s[2]:.4f}\t{s[3]:.4f}\t{s[4]:.4f}\t{s[5]:.4f}\n")
            ...
            for epoch in range(N_EPOCHS):
                ...
                for batch_start in range(0, N_SAMPLES, BATCH_SIZE):
                    ...
                    for test_data_idx in range(batch_start, batch_end):
                        ...
                        state = (state_orig[0], state_orig[2], state_orig[1])
                        prev_state = None
                        prev_triggered = None
                        prev_reward = None
    ...
    def __init__(self, target_path, swarm_size=20, dqn_samples=None):
        ...
        if dqn_samples is not None and len(dqn_samples) > 0:
            ...
            for i in range(num_direct):
                ...
                # state_tuple: (config_depth, param_count, security_weight) -> 转为 (light, temp, moisture)
                state_for_pso = (state_tuple[0], state_tuple[2], state_tuple[1])
                particle = Particle(initial_position=state_for_pso)
            ...
            if len(self.particles) < swarm_size:
                ...
                for i in range(remaining):
                    ...
                    perturbed = np.array([state_tuple[0], state_tuple[2], state_tuple[1]]) + np.random.uniform(-1, 1, size=3)
...
def export_multiple_runs_to_excel(all_run_results, all_run_metrics, num_runs, filename=None):
    ...
    headers3 = ["路径", "运行", "最优位置(config_depth, param_count, security_weight)", "适应度", "迭代次数", "方法", "触发规则集合"]
    ...
    for path_idx in range(num_paths):
        for run_idx, results in enumerate(all_run_results, start=1):
            ...
            # 将 (light, temp, moisture) 转换回 (config_depth, param_count, security_weight)
            config_depth = best_pos[0]
            security_weight = best_pos[1]
            param_count = int(best_pos[2])
            particle_str = f"({config_depth:.2f}, {param_count}, {security_weight:.3f})"
...
def run_multiple_dqn_pso_experiments(num_runs):
    ...
    print(f"范围: config_depth: {BOUNDS['light']}, param_count: {BOUNDS['moisture']}, security_weight: {BOUNDS['temp']}")
    print("=" * 100)
...
if __name__ == "__main__":
    print("=" * 100)
    print("DQN-PSO (标准PSO) - 第二个单元测试 (配置验证规则)")
    print("=" * 100)
