...
# === 动作增量（沿用原有数值，执行时自动裁剪） ===
DELTA_CONFIG = [35, 25, 10, 5, 2, -2, -5, -10, -25, -35]
DELTA_PARAM = [35, 25, 10, 5, 2, -2, -5, -10, -25, -35]
DELTA_SECURITY = [35, 25, 10, 5, 2, -2, -5, -10, -25, -35]   # 实际会被裁剪到 [0,1]
...
def generate_random_state():
    """生成随机状态（整数或浮点数）"""
    config = np.random.uniform(BOUNDS['config_depth'][0], BOUNDS['config_depth'][1])
    param = np.random.randint(BOUNDS['param_count'][0], BOUNDS['param_count'][1] + 1)
    security = np.random.uniform(BOUNDS['security_weight'][0], BOUNDS['security_weight'][1])
    return np.array([config, param, security])
...
def is_state_valid(state):
    """检查状态是否在边界内"""
    return (BOUNDS['config_depth'][0] <= state[0] <= BOUNDS['config_depth'][1] and
            BOUNDS['param_count'][0] <= state[1] <= BOUNDS['param_count'][1] and
            BOUNDS['security_weight'][0] <= state[2] <= BOUNDS['security_weight'][1])
...
def generate_samples_for_similar_paths(similar_group_indices, num_total=2000, top_k=200):
    ...
    def save_samples(path_id, samples, base_dir):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            for s in samples:
                config, param, sec = s[0][0], s[0][1], s[0][2]
                f.write(f"{config:.3f} {param} {sec:.3f}\t{s[1]:.4f}\t{s[2]:.4f}\t{s[3]:.4f}\t{s[4]:.4f}\n")
...
def load_path_data(file_path):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        for line in lines[2:]:
            ...
            if parts:
                # 读取三个数值，config_depth 和 security_weight 可能是浮点数
                vals = parts[0].split()
                if len(vals) == 3:
...
class DQNAgentWithPER:
    ...
    def decode_action(self, action_idx):
        """将动作索引解码为增量 (d_config, d_param, d_security)"""
        dim = action_idx // 10
        delta_idx = action_idx % 10
        if dim == 0:
            delta = DELTA_CONFIG[delta_idx]
            return (delta, 0, 0)
        elif dim == 1:
            delta = DELTA_PARAM[delta_idx]
            return (0, delta, 0)
        elif dim == 2:
            delta = DELTA_SECURITY[delta_idx]
            return (0, 0, delta)
...
def generate_samples_for_isolated_paths(agent_similar, isolated_group_indices, num_total=2000, top_k=200):
    ...
    def save_samples(path_id, samples, base_dir):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write("config_depth param_count security_weight\tScore\tSimilarity\tLengthDiff\tRobustness\tQ_normalized_complement\n")
            for s in samples:
                config, param, sec = s[0][0], s[0][1], s[0][2]
                f.write(f"{config:.3f} {param} {sec:.3f}\t{s[1]:.4f}\t{s[2]:.4f}\t{s[3]:.4f}\t{s[4]:.4f}\t{s[5]:.4f}\n")
    ...
    def update(self, iteration, max_iterations):
        ...
        for particle in self.particles:
            ...
            max_velocity = np.array([
                (BOUNDS['config_depth'][1] - BOUNDS['config_depth'][0]) * 0.2,
                (BOUNDS['param_count'][1] - BOUNDS['param_count'][0]) * 0.2,
                (BOUNDS['security_weight'][1] - BOUNDS['security_weight'][0]) * 0.2
            ])
...
def export_multiple_runs_to_excel(all_run_results, all_run_metrics, num_runs, filename=None):
    ...
    headers3 = ["Path", "Run", "(config_depth, param_count, security_weight)", "适应度", "迭代次数", "方法", "触发路径"]
...
def run_multiple_dqn_pso_experiments(num_runs):
    ...
    print(f"搜索空间: config_depth={BOUNDS['config_depth']}, param_count={BOUNDS['param_count']}, security_weight={BOUNDS['security_weight']}")
    print("="*100)
