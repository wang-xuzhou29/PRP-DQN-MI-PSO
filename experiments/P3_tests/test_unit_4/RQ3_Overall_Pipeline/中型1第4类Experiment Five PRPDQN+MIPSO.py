...
# === 第四个单元测试的范围（threat_count, severity_level, confidence_score）===
# 维度1: threat_count 1~120 (int)
LIGHT_MIN = 1
LIGHT_MAX = 120
# 维度2: severity_level 1~10 (float)  —— 对应原 temp 维度
TEMP_MIN = 1.0
TEMP_MAX = 10.0
# 维度3: confidence_score 1~100 (int) —— 对应原 moisture 维度
MOISTURE_MIN = 1
MOISTURE_MAX = 100

BOUNDS = {
    'light': (LIGHT_MIN, LIGHT_MAX),          # threat_count
    'temp': (TEMP_MIN, TEMP_MAX),             # severity_level
    'moisture': (MOISTURE_MIN, MOISTURE_MAX)  # confidence_score
}
...
def execute_Tr(position):
    """
    执行目标函数，返回触发的规则集合
    参数position: (threat_count, severity_level, confidence_score)
    """
    threat_count = int(np.clip(position[0], BOUNDS['light'][0], BOUNDS['light'][1]))
    severity_level = float(np.clip(position[1], BOUNDS['temp'][0], BOUNDS['temp'][1]))
    confidence_score = int(np.clip(position[2], BOUNDS['moisture'][0], BOUNDS['moisture'][1]))
    return execute_threat_analysis_rules(threat_count, severity_level, confidence_score)


def execute_threat_analysis_rules(threat_count, severity_level, confidence_score):
    """
    威胁分析规则函数（来自第四个单元测试）
    参数：threat_count (int), severity_level (float), confidence_score (int)
    返回触发规则集合
    """
    triggered = set()
    b = {}

    # 以下代码从用户提供的 execute_threat_analysis_rules 完整复制
...
def generate_samples_for_similar_paths(similar_group_indices, num_total=2000, top_k=200):
    ...
    def compute_robustness(state, path):
        ...
        if not base:
            return 0.0
        rob, neighbors = 0.0, 0
        for dx in [-1, 0, 1]:          # threat_count 扰动
            for dy in [-0.1, 0, 0.1]:  # severity_level 扰动
                for dz in [-1, 0, 1]:  # confidence_score 扰动
    ...
    def save_samples(path_id, samples, base_dir):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            for s in samples:
                threat_count, severity_level, confidence_score = int(s[0][0]), s[0][1], int(s[0][2])
                f.write(f"{threat_count} {severity_level:.2f} {confidence_score}\t{s[1]:.4f}\t{s[2]:.4f}\t{s[3]:.4f}\t{s[4]:.4f}\n")
...
class SharedExperienceReplay:
    ...
    def get_high_reward_samples(self, target_path, num_samples=20):
        ...
        for experience in self.buffer:
            ...
            # 存储顺序 (threat_count, severity_level, confidence_score)
            state_tuple = (int(state_np[0]), state_np[1], int(state_np[2]))
...
def load_path_data(file_path):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        for line in lines[2:]:
            ...
            if parts:
                vals = parts[0].split()
                threat_count = int(vals[0])
                severity_level = float(vals[1])
                confidence_score = int(vals[2])
                path_data.append((threat_count, severity_level, confidence_score))
    return path_data
...
class DQNAgentWithPER:
    ...
    def decode_action(self, action_idx):
        """解码动作：0-9 为 threat_count，10-19 为 severity_level，20-29 为 confidence_score"""
        dim = action_idx // 10
        delta_idx = action_idx % 10

        if dim == 0:  # threat_count
            delta = DELTA_LIGHT[delta_idx]
            return (delta, 0, 0)
        elif dim == 1:  # severity_level
            delta = DELTA_TEMP[delta_idx]
            return (0, delta, 0)
        elif dim == 2:  # confidence_score
            ...
            for epoch in range(N_EPOCHS):
                ...
                for batch_start in range(0, N_SAMPLES, BATCH_SIZE):
                    ...
                    for test_data_idx in range(batch_start, batch_end):
                        ...
                        # state_orig: (threat_count, severity_level, confidence_score) 直接使用
                        state = state_orig
                        prev_state = None
                        prev_triggered = None
                        prev_reward = None
...
def generate_samples_for_isolated_paths(agent_similar, isolated_group_indices, num_total=2000, top_k=200):
    ...
    def save_samples(path_id, samples, base_dir):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write("threat_count severity_level confidence_score\tScore\tSimilarity\tLengthDiff\tRobustness\tQ_value_normalized_complement\n")
            for s in samples:
                threat_count, severity_level, confidence_score = int(s[0][0]), s[0][1], int(s[0][2])
                f.write(f"{threat_count} {severity_level:.2f} {confidence_score}\t{s[1]:.4f}\t{s[2]:.4f}\t{s[3]:.4f}\t{s[4]:.4f}\t{s[5]:.4f}\n")
    ...
    def __init__(self, initial_position=None):
        if initial_position is not None:
            ...
            self.position = np.array([
                np.random.randint(BOUNDS['light'][0], BOUNDS['light'][1] + 1),
    ...
    def __init__(self, target_path, swarm_size=20, dqn_samples=None):
        ...
        if dqn_samples is not None and len(dqn_samples) > 0:
            ...
            for i in range(num_direct):
                ...
                # state_tuple: (threat_count, severity_level, confidence_score) 直接使用
            ...
            if len(self.particles) < swarm_size:
                ...
                for i in range(remaining):
                    ...
                    perturbed[0] = int(perturbed[0])  # threat_count 整数
                    perturbed[2] = int(perturbed[2])  # confidence_score 整数
...
def export_multiple_runs_to_excel(all_run_results, all_run_metrics, num_runs, filename=None):
    ...
    headers3 = ["路径", "运行", "最优位置(threat_count, severity_level, confidence_score)", "适应度", "迭代次数", "方法", "触发规则集合"]
    col_widths3 = [10, 10, 35, 12, 12, 12, 50]
    ...
    for path_idx in range(num_paths):
        for run_idx, results in enumerate(all_run_results, start=1):
            ...
            threat_count = int(best_pos[0])
            severity_level = best_pos[1]
            confidence_score = int(best_pos[2])
            particle_str = f"({threat_count}, {severity_level:.2f}, {confidence_score})"
...
def run_multiple_dqn_pso_experiments(num_runs):
    ...
    print(f"范围: threat_count: {BOUNDS['light']}, severity_level: {BOUNDS['temp']}, confidence_score: {BOUNDS['moisture']}")
    print("=" * 100)
...
if __name__ == "__main__":
    print("=" * 100)
    print("DQN-PSO (标准PSO) - 第四个单元测试 (威胁分析规则)")
    print("=" * 100)
