...
# 动作步长：threat_count 步长 1，severity_level 步长 0.1，confidence_score 步长 1.0
STEP_THREAT = 1
STEP_SEVERITY = 0.1
STEP_CONFIDENCE = 1.0
...
def normalize_state(state):
    """归一化到 [0,1]"""
    x, y, z = state
    norm_x = (x - THREAT_COUNT_MIN) / (THREAT_COUNT_MAX - THREAT_COUNT_MIN)
    norm_y = (y - SEVERITY_LEVEL_MIN) / (SEVERITY_LEVEL_MAX - SEVERITY_LEVEL_MIN)
    norm_z = (z - CONFIDENCE_SCORE_MIN) / (CONFIDENCE_SCORE_MAX - CONFIDENCE_SCORE_MIN)
    return [norm_x, norm_y, norm_z]

def denormalize_state(normalized_state):
    """反归一化：threat_count 取整，severity_level 和 confidence_score 保留两位小数"""
    nx, ny, nz = normalized_state
    x = int(np.clip(nx * (THREAT_COUNT_MAX - THREAT_COUNT_MIN) + THREAT_COUNT_MIN,
                    THREAT_COUNT_MIN, THREAT_COUNT_MAX))
    y = np.clip(ny * (SEVERITY_LEVEL_MAX - SEVERITY_LEVEL_MIN) + SEVERITY_LEVEL_MIN,
                SEVERITY_LEVEL_MIN, SEVERITY_LEVEL_MAX)
    y = round(y, 2)
    z = np.clip(nz * (CONFIDENCE_SCORE_MAX - CONFIDENCE_SCORE_MIN) + CONFIDENCE_SCORE_MIN,
                ...
                for sample_idx in range(batch_start, batch_end):
                    ...
                    for step in range(N_STEPS):
                        ...
                        for a in range(agent.action_dim):
                            ...
                            if (THREAT_COUNT_MIN <= cand_next[0] <= THREAT_COUNT_MAX and
                                SEVERITY_LEVEL_MIN <= cand_next[1] <= SEVERITY_LEVEL_MAX and
                                CONFIDENCE_SCORE_MIN <= cand_next[2] <= CONFIDENCE_SCORE_MAX):
                        ...
                        next_state = (state[0] + dt, state[1] + ds, state[2] + dc)
                        next_state = (next_state[0], round(next_state[1], 2), round(next_state[2], 2))
...
def create_consolidated_excel_report(all_runs_data, similar_group, isolated_group, output_dir):
    ...
    output_path = os.path.join(output_dir, "20次运行_威胁分析.xlsx")
    wb.save(output_path)
...
def run_20_times_training():
    ...
    print(f"范围: threat_count[{THREAT_COUNT_MIN},{THREAT_COUNT_MAX}], severity_level[{SEVERITY_LEVEL_MIN},{SEVERITY_LEVEL_MAX}], confidence_score[{CONFIDENCE_SCORE_MIN},{CONFIDENCE_SCORE_MAX}]")
    ...
    print("\n" + "=" * 60)
    print("20次运行全部完成！ - 威胁分析规则")
    print("=" * 60)
    print(f"状态空间范围:")
    print(f"  threat_count: [{THREAT_COUNT_MIN}, {THREAT_COUNT_MAX}]")
    print(f"  severity_level: [{SEVERITY_LEVEL_MIN}, {SEVERITY_LEVEL_MAX}]")
    print(f"  confidence_score: [{CONFIDENCE_SCORE_MIN}, {CONFIDENCE_SCORE_MAX}]")
