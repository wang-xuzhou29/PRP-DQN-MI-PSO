...
# ==================== 威胁分析专用范围 ====================
STATE_RANGES = {
    'threat_count': (1, 120),          # 整数
    'severity_level': (1.0, 10.0),     # 浮点数，保留两位小数
    'confidence_score': (1.0, 100.0)   # 浮点数，保留两位小数
}
STATE_NAMES = ('threat_count', 'severity_level', 'confidence_score')
# 使用float类型以便支持浮点数
...
def clip_state(state):
    """裁剪状态到合法范围，返回 tuple（第一个整数，后两个保留两位小数）"""
    state = np.array(state, dtype=np.float32)
    clipped = np.clip(state, STATE_MIN, STATE_MAX)
    clipped[0] = int(round(clipped[0]))
    clipped[1] = round(clipped[1], 2)
    clipped[2] = round(clipped[2], 2)
    return tuple(clipped)

def random_state():
    """生成随机状态：第一个整数，后两个浮点数（两位小数）"""
    tc = random.randint(int(STATE_RANGES['threat_count'][0]), int(STATE_RANGES['threat_count'][1]))
    sl = round(random.uniform(STATE_RANGES['severity_level'][0], STATE_RANGES['severity_level'][1]), 2)
    cs = round(random.uniform(STATE_RANGES['confidence_score'][0], STATE_RANGES['confidence_score'][1]), 2)
    return (tc, sl, cs)
...
class StateNormalizer:
    ...
    def denormalize(self, normalized_state):
        ...
        denormalized[1] = round(np.clip(denormalized[1], low, high), 2)
...
def execute_threat_analysis_rules(a):
    """
    参数a: 包含3个元素的元组或数组，对应 threat_count, severity_level, confidence_score
    返回: 触发的规则编号集合
    """
...
def compute_robustness(state, path):
    """计算鲁棒性：邻域平均Jaccard相似度（适配威胁分析，步长：tc ±1, sl ±0.1, cs ±1）"""
    tc, sl, cs = state
    base = execute_Tr(tc, sl, cs)
    if not base:
        return 0.0
    ...
    def save_samples(path_id, samples, base_dir, group_type="similar"):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            for s in samples:
                tc, sl, cs = s['state']
                f.write(
                    f"{tc} {sl:.2f} {cs:.2f}\t{s['score']:.4f}\t{s['similarity']:.4f}\t{s['length_diff']:.4f}\t{s['robustness']:.4f}\n")
    ...
    def save_samples(path_id, samples, base_dir, group_type="isolated"):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            for s in samples:
                tc, sl, cs = s['state']
                f.write(
                    f"{tc} {sl:.2f} {cs:.2f}\t{s['score']:.4f}\t{s['similarity']:.4f}\t{s['length_diff']:.4f}\t{s['robustness']:.4f}\t{s['q_value_score']:.4f}\n")
...
class DQNAgentWithPER:
    ...
    def decode_action(self, action_idx):
        """解码动作：每个维度步长 ±1，但实际应用时对 severity 乘以 0.1，对 confidence 乘以 1"""
        delta_values = [1, -1]
        dim = action_idx // 2
        delta_idx = action_idx % 2
        delta = delta_values[delta_idx]
        if dim == 0:
            return (delta, 0, 0)           # threat_count
        elif dim == 1:
            return (0, delta * 0.1, 0)     # severity_level 步长 ±0.1
        elif dim == 2:
            return (0, 0, delta)           # confidence_score 步长 ±1
...
def train_group(group_paths, path_documents, replay_buffer, batch_size=32, group_name="", pretrained_model=None,
    ...
    if pretrained_model is not None:
        print(f"  {group_name}: loading pretrained model...")
    ...
    for path_idx in group_paths:
        ...
        if not os.path.exists(file_path):
            print(f"    : Path {path_idx + 1}, file missing, skipping")
            continue
        ...
        print(f"\n  Start training path  {path_idx + 1}, {N_ROUNDS} rounds")
        ...
        for round_idx in range(N_ROUNDS):
            ...
            for batch_idx in range(N_BATCHES):
                ...
                if batch_start >= len(path_data):
                    print(f"       {batch_idx + 1}: insufficient samples, skipping")
                    break

                print(f"       {batch_idx + 1}/{N_BATCHES} (samples {batch_start}-{batch_end})")
        ...
        print(f"  Path  {path_idx + 1} {N_ROUNDS} rounds completed")

    training_time = time.time() - start_time
    print(f"\n{group_name} training completed!")
    print(f"  Paths completed: {N_ROUNDS} rounds each")
    print(f"  Replay updates: {replay_count}")
...
def generate_and_train_grouped_paths_staged(path_documents, random_group1, random_group2, batch_size=32, run_id=1):
    print(f"\n===  Run {run_id}/20 (random grouping + model reuse) ===")
    random_group1_paths = [idx + 1 for idx in random_group1]
    random_group2_paths = [idx + 1 for idx in random_group2]

    print(f"Random group1 (pretrained group): {random_group1_paths}")
    print(f"Random group2 (model-reuse group): {random_group2_paths}")

    total_start_time = time.time()

    print(f"\n[1] Generating samples for random group1...")
    generate_samples_for_similar_paths(random_group1, num_candidates=2000, top_k=200, run_id=run_id)

    print(f"\n[2] Training random group1 (pretrained)...")
    ...
    group1_agent, group1_path_rewards, group1_training_time = train_group(
        ...
        group_name="Random group1 (pretrained)", pretrained_model=None, sample_group_type="similar"
    )

    print(f"\n[3] Generating samples for random group2 using model from group1...")
    generate_samples_for_isolated_paths(random_group2, group1_agent.model,
                                        num_candidates=2000, top_k=200, run_id=run_id)

    print(f"\n[4] Training random group2 (model reuse from group1)...")
    ...
    group2_agent, group2_path_rewards, group2_training_time = train_group(
        ...
        group_name="Random group2 (model-reuse)", pretrained_model=group1_agent.model, sample_group_type="isolated"
    )
    ...
    print(f"Random group1 training time: {group1_training_time:.2f} sec")
    print(f"Random group2 training time: {group2_training_time:.2f} sec")
    print(f"Replay buffer sizes - group1: {len(group1_replay_buffer)}, group2: {len(group2_replay_buffer)}")
...
def create_consolidated_excel_report(all_runs_data, similar_group, isolated_group, output_dir):
    ...
    for path_id in range(1, len(target_paths) + 1):
        ...
        if path_id in similar_group_paths:
            group_type = "Random group1 (pretrained)"
            row_color = similar_group_color
        elif path_id in isolated_group_paths:
            group_type = "Random group2 (model-reuse)"
    ...
    cell = ws_groups.cell(row=row, column=1, value="Random group1 (pretrained)")
    ...
    if isolated_group_paths:
        cell = ws_groups.cell(row=row, column=1, value="Random group2 (model-reuse)")
        cell.font = Font(bold=True, size=11)
        cell.alignment = Alignment(horizontal="center", vertical="center")
    ...
    sample_headers = ['Run', 'Path ID', 'Sample ID', 'Threat_Count', 'Severity_Level', 'Confidence_Score', 'Similarity', 'Triggered Rules']
    ...
    output_path = os.path.join(output_dir, "20_runs_threat_random_reuse.xlsx")
    wb.save(output_path)
    print(f"\n Consolidated Excel report generated: {output_path}")

def run_20_times_training():
    model_path_base = r"D:\Experiment\CNN\DQNNEW\saved_models_threat_random_reuse"
    path_documents = r"D:\Experiment\CNN\DQNNEW\path_samples_grouped"
    output_dir = r"D:\Experiment\CNN\ComparisonExperiment2\excel_reports_threat_random_reuse"
    ...
    print("=" * 60)
    print("20 runs - Threat analysis rules + random grouping + model reuse")
    print(f"State ranges: threat_count[1,120], severity_level[1.0,10.0], confidence_score[1.0,100.0]")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("20 runs all completed! - Threat analysis + random grouping + model reuse")
    print("=" * 60)
    print(f"Summary:")
