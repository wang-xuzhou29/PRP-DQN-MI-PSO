...
EXPERIMENT_CONFIG = {
    'STATE_DIM': 3,
    'MIN_X': 1, 'MAX_X': 120,         # threat_count
    'MIN_Y': 1.0, 'MAX_Y': 10.0,      # severity_level (浮点)
    'MIN_Z': 1.0, 'MAX_Z': 100.0,     # confidence_score (浮点)
...
def denormalize_state(normalized_state):
    ...
    # threat_count 取整，severity_level 和 confidence_score 保留两位小数
    state[0] = np.round(state[0]).astype(int)
...
class ImprovedDQNAgent:
    ...
    def discrete_to_action_delta(self, action_idx):
        ...
        delta = delta_lists[dim][delta_idx]
        ...
        for path_idx in range(len(target_paths)):
            ...
            for sample_idx, sample in enumerate(samples):
                ...
                dqn_detailed_data.append({
                    ...
                    'Threat_Count': int(state[0]),
                    'Severity_Level': round(state[1], 2),
                    'Confidence_Score': round(state[2], 2),
    ...
    for path_idx in range(num_paths):
        ...
        for _ in range(EXPERIMENT_CONFIG['SAMPLES_PER_PATH']):
            state = np.array([
                ...
                round(random.uniform(EXPERIMENT_CONFIG['MIN_Y'], EXPERIMENT_CONFIG['MAX_Y']), 2),
                round(random.uniform(EXPERIMENT_CONFIG['MIN_Z'], EXPERIMENT_CONFIG['MAX_Z']), 2)
            ], dtype=np.float32)
...
def main():
    print("\n" + "=" * 80)
    print("DQN - 20次运行实验 (威胁分析规则)")
    print("性能指标：覆盖相似度")
    print(f"状态范围: threat_count[{EXPERIMENT_CONFIG['MIN_X']}-{EXPERIMENT_CONFIG['MAX_X']}], "
          f"severity_level[{EXPERIMENT_CONFIG['MIN_Y']}-{EXPERIMENT_CONFIG['MAX_Y']}], "
          f"confidence_score[{EXPERIMENT_CONFIG['MIN_Z']}-{EXPERIMENT_CONFIG['MAX_Z']}]")
    print("=" * 80)
    ...
    output_path = f"DQN_20_runs_threat_{timestamp}.xlsx"
