...
EXPERIMENT_CONFIG = {
    'STATE_DIM': 3,
    'ACTION_DIM': 3,
    # 测试范围：threat_count[1,120], severity_level[1,10], confidence_score[1,100]
    'MIN_VALUES': np.array([1, 1.0, 1.0], dtype=np.float32),
    'MAX_VALUES': np.array([120, 10.0, 100.0], dtype=np.float32),
    ...
    'TARGET_PATHS': [
        # A1
        {6, 7, 8, 11, 13, 17, 19, 21, 28, 29, 30, 32, 34, 37, 39, 42, 49, 52, 57, 58, 63, 64, 65, 66, 73, 75, 80, 82, 90,
         91},
        # A2
...
def execute_threat_analysis_rules(a):
    """
    威胁分析规则执行函数
    参数a: 包含3个元素的数组，对应 threat_count, severity_level, confidence_score
    返回: 触发的规则编号集合
    """
    ...
    def get_top_k_per_path(self, num_paths, k=20):
        ...
        for path_idx in range(num_paths):
            ...
            for sample in top_k:
                ...
                # threat_count取整，severity_level和confidence_score保留浮点
...
def train_sac_workflow():
    ...
    for path_idx in range(num_paths):
        ...
        for _ in range(EXPERIMENT_CONFIG['SAMPLES_PER_PATH']):
            # threat_count整数，severity_level浮点，confidence_score浮点
            state = np.array([
        ...
        for path_idx in range(num_paths):
            ...
            for sample_idx, initial_state in enumerate(batch_samples):
                ...
                for step in range(EXPERIMENT_CONFIG['STEPS_PER_SAMPLE']):
                    ...
                    reward = unified_reward_function(triggered, target_path)
                    similarity = coverage_similarity(triggered, target_path)
                    done = 1.0 if (step == EXPERIMENT_CONFIG['STEPS_PER_SAMPLE'] - 1) else 0.0
...
def main():
    print("\n" + "=" * 80)
    print("SAC - 20次运行实验 (威胁分析规则)")
    print("性能指标：覆盖相似度")
    min_vals = EXPERIMENT_CONFIG['MIN_VALUES']
    max_vals = EXPERIMENT_CONFIG['MAX_VALUES']
    print(f"状态范围: threat_count[{int(min_vals[0])}-{int(max_vals[0])}], "
          f"severity_level[{min_vals[1]}-{max_vals[1]}], "
          f"confidence_score[{min_vals[2]}-{max_vals[2]}]")
    print("=" * 80)
    ...
    output_path = f"SAC_20_runs_threat_analysis_{timestamp}.xlsx"
