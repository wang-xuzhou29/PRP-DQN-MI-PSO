...
def normalize_state(state):
    """将状态归一化到 [0, 1] 区间"""
    x_norm = (state[0] - MIN_X) / (MAX_X - MIN_X)
    y_norm = (state[1] - MIN_Y) / (MAX_Y - MIN_Y)
    z_norm = (state[2] - MIN_Z) / (MAX_Z - MIN_Z)
    return (x_norm, y_norm, z_norm)

def denormalize_state(state_norm):
    """将归一化状态还原"""
    x = int(round(state_norm[0] * (MAX_X - MIN_X) + MIN_X))
    y = int(round(state_norm[1] * (MAX_Y - MIN_Y) + MIN_Y))
...
def compute_robustness(state, path, sample_size=9):
    ...
    for dw, dt, dz_val in deltas[:sample_size]:
        if dw == dt == dz_val == 0:
            continue

        neighbor_x = int(np.clip(state[0] + dw, MIN_X, MAX_X))
        neighbor_y = int(np.clip(state[1] + dt, MIN_Y, MAX_Y))
        neighbor_z = int(np.clip(state[2] + dz_val, MIN_Z, MAX_Z))
...
def generate_samples_for_all_paths(num_candidates=2000, top_k=200, run_id=1):
    ...
    def save_samples(path_id, samples, base_dir):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write("x y z\tScore\tSimilarity\tRobustness\tLengthDiff\n")
            for s in samples:
                x, y, z = s['state']
                f.write(
                    f"{x} {y} {z}\t{s['score']:.4f}\t{s['similarity']:.4f}\t"
    ...
    for path_idx in range(len(targetPaths)):
        ...
        while len(candidate_samples) < num_candidates and attempts < max_attempts:
            ...
            x_val = np.random.randint(MIN_X, MAX_X + 1)
            y_val = np.random.randint(MIN_Y, MAX_Y + 1)
            z_val = np.random.randint(MIN_Z, MAX_Z + 1)
            state = (x_val, y_val, z_val)

            triggered = execute_Tr(x_val, y_val, z_val)
            if not triggered:
                continue
...
class DQNAgentWithPER:
    ...
    def decode_action(self, action_idx):
        ...
        if dim == 0:  # x
            if delta_idx >= 4:
                delta_idx = 3
            return (delta_values_xy[delta_idx], 0, 0)
        elif dim == 1:  # y
            if delta_idx >= 4:
                delta_idx = 3
            return (0, delta_values_xy[delta_idx], 0)
        elif dim == 2:  # z
            if delta_idx >= 8:
                delta_idx = 7
    ...
    def get_legal_actions(self, state):
        ...
        for action_idx in range(self.action_dim):
            dx, dy, dz = self.decode_action(action_idx)

            next_x = state[0] + dx
            next_y = state[1] + dy
            next_z = state[2] + dz

            if (MIN_X <= next_x <= MAX_X and
                    MIN_Y <= next_y <= MAX_Y and
                    MIN_Z <= next_z <= MAX_Z):
            ...
            for batch_idx in range(NUM_BATCHES):
                ...
                for sample_idx in range(batch_start, batch_end):
                    ...
                    for step in range(STEPS_PER_SAMPLE):
                        ...
                        next_state = (
                            int(np.clip(state[0] + dx, MIN_X, MAX_X)),
                            int(np.clip(state[1] + dy, MIN_Y, MAX_Y)),
...
def create_consolidated_excel_report(all_runs_data, similar_group, isolated_group, output_dir):
    ...
    sample_headers = ['Run', 'Path ID', 'Sample ID', 'X', 'Y', 'Z', 'Similarity', 'Triggered Rule Set']
    ...
    for run_idx, run_data in enumerate(all_runs_data, 1):
        for path_id in range(1, NUM_PATHS + 1):
            ...
            for sample_idx, (state_tuple, reward, sim, triggered) in enumerate(samples, 1):
                ...
                for col_offset, value in enumerate([x, y, z]):
    ...
    summary_headers = ['Run', 'Training Time( seconds)', 'Overall Average Similarity', 'Maximum Similarity',
                       'Minimum Similarity', 'High Group Avg', 'Low Group Avg', 'Shared Buffer Size']
