...
dy_min, dy_max = 1, 150  # dy范围
dz_min, dz_max = 1, 200  # dz范围
...
def denormalize_state(normalized_state):
    ...
    dx = int(round(norm_dx * (dx_max - dx_min) + dx_min))
    dy = int(round(norm_dy * (dy_max - dy_min) + dy_min))
    dz = int(round(norm_dz * (dz_max - dz_min) + dz_min))
    return (dx, dy, dz)
