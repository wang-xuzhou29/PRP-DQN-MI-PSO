...
# === 全局取值范围配置（按您最后要求：X:1~65, Y:100~205, Z:1~20） ===
STATE_MIN_W, STATE_MAX_W = 1, 65      # 温度/湿度
STATE_MIN_T, STATE_MAX_T = 100, 205   # 电压/扭矩
STATE_MIN_Z, STATE_MAX_Z = 1, 20      # 流量/电流
# === / ===
...
# ================== 规则函数 section10（已修正裁剪范围） ==================
def section10_comprehensive_hybrid_control(x, y, z):
    """第10类: 综合混合控制 (湿度x, 扭矩y, 电流z) - 158个分支（删除99、100后顺延编号）"""
    # 使用全局范围裁剪
    x = max(STATE_MIN_W, min(STATE_MAX_W, x))
    y = max(STATE_MIN_T, min(STATE_MAX_T, y))
...
# ======================== 关键修正：execute_Tr 指向 section10 ========================
execute_Tr = section10_comprehensive_hybrid_control


# ======================== 目标路径组（重要：必须与 section10 规则编号匹配） ========================
# 警告：以下 targetPaths 是为 section9 设计的（编号 1~160），与 section10（编号 1~156）不完全匹配。
# 您必须根据 section10 的规则（1~156）重新设计目标路径集合，否则实验将失去意义。
# 这里保留原列表，但建议您替换为正确的路径。
# 本示例中暂不修改，您可以在运行前替换。
targetPaths = [
