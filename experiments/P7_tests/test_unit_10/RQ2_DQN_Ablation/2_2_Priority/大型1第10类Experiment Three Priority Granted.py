...
# ========== 新的取值范围（适应 section10） ==========
X_MIN, X_MAX = 1.0, 65.0      # 湿度范围
Y_MIN, Y_MAX = 100.0, 205.0   # 扭矩范围
Z_MIN, Z_MAX = 1.0, 20.0      # 电流范围
# ======================================================
...
def section10_comprehensive_hybrid_control(x, y, z):
    """第10类: 综合混合控制 (湿度x, 扭矩y, 电流z) - 156个有效分支（删除了原编号99、100）"""
    triggered = set()
    ...
    # -------------------------- 81-156：全系统优化协调（原编号99、100被删除，后续顺延） --------------------------
    ...
    # 原99、100被删除，因此从101开始顺延
    if [(min(x / 50, y / 175, z / 12.5) > 0.985)] != [(min(x / 50, y / 175, z / 12.5) > 9.85)]:
        triggered.add(99)
    if [(min(x / 50, y / 175, z / 12.5) > 0.985)] != [(min(x / 50, y / 175, z / 12.5) > 9.85)]:
        triggered.add(100)   # 注意：第二个表达式与第一个相同，但代码中就是如此（原100）
...
# ========== execute_Tr 指向 section10 ==========
def execute_Tr(x, y, z):
    return section10_comprehensive_hybrid_control(x, y, z)
...
# === 目标路径组（与 section10 匹配，此处只保留前几个作为示例，实际请使用您提供的完整列表） ===
targetPaths = [
    ...
    # ... 请将您提供的完整 targetPaths 列表粘贴在此处（所有28个集合）
    # 由于篇幅，此处省略，实际使用时请替换为完整列表
]
