...
# === 全局取值范围（新范围：x:1~100, y:1~100, z:1~60） ===
MIN_X = 1
MAX_X = 100
MIN_Y = 1
MAX_Y = 100
...
# ========== 规则触发函数（section6_thermal_electrical_flow_hybrid） ==========
def section6_thermal_electrical_flow_hybrid(x, y, z):
    """第6类：热-电-流混合控制（x:温度/光照, y:电压/扭矩, z:流量/电流因子）"""
    triggered = set()
...
def compute_robustness(state, path, sample_size=9):
    """计算鲁棒性"""
    base = section6_thermal_electrical_flow_hybrid(state[0], state[1], state[2])
    if not base:
        return 0.0
    ...
    for dw, dt, dz in deltas[:sample_size]:
        ...
        n_trig = section6_thermal_electrical_flow_hybrid(neighbor[0], neighbor[1], neighbor[2])
        if not n_trig:
            continue
...
def generate_samples_for_all_paths(num_candidates=2000, top_k=200, run_id=1):
    ...
    for path_idx in range(len(target_paths)):
        ...
        while len(candidate_samples) < num_candidates and attempts < max_attempts:
            ...
            triggered = section6_thermal_electrical_flow_hybrid(weather, time_period, z)
            if not triggered:
                continue
...
class SharedExperienceReplay:
    ...
    def get_high_reward_samples(self, target_path, num_samples=20):
        ...
        for experience in self.buffer:
            ...
            triggered = section6_thermal_electrical_flow_hybrid(state_tuple[0], state_tuple[1], state_tuple[2])
            ...
            for batch_idx in range(NUM_BATCHES):
                ...
                for sample_idx in range(batch_start, batch_end):
                    ...
                    for step in range(STEPS_PER_SAMPLE):
                        ...
                        triggered = section6_thermal_electrical_flow_hybrid(next_state[0], next_state[1], next_state[2])
